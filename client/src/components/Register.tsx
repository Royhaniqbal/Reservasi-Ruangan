    ...
    <div className="flex min-h-screen w-screen relative">
      ...
      <div className="hidden md:flex w-full">
        ...
        <div className="w-3/5 bg-white flex flex-col justify-center items-center space-y-10">
          <img src="/logokemnaker.png" alt="Kemnaker" className="h-80 select-none pointer-events-none" draggable={false} onContextMenu={(e) => e.preventDefault()} />
          <hr className="w-2/3 border-t-2 border-blue-700" />
          <img src="/logovokasi.png" alt="Pelatihan Vokasi" className="h-40 select-none pointer-events-none" draggable={false} onContextMenu={(e) => e.preventDefault()} />
        </div>
        ...
        <div className="w-2/5 bg-blue-700 flex flex-col justify-center items-center text-white px-10">
          <div className="max-w-xl w-full flex flex-col items-center">
            <div className="mb-16">
              ...
              <p className="text-center text-2xl">Sistem Informasi Pelayanan Peminjaman Ruangan</p>
            </div>
            ...
            <form onSubmit={handleRegister} className="space-y-4 flex flex-col items-center w-full">
              ...
              <input type="text" placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} className="w-7/12 px-3 py-2 rounded-full border bg-white border-gray-300 text-black text-lg" required />
              <input type="email" placeholder="E-Mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-7/12 px-3 py-2 rounded-full border bg-white border-gray-300 text-black text-lg" required />
              ...
              {/* Role */}
              <div className="relative w-7/12">
                <select value={role} onChange={(e) => setRole(e.target.value)} className="appearance-none w-full px-5 py-2 rounded-full border border-gray-300 bg-white text-black text-lg">
                  <option value="user">👤 User</option>
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-6 flex items-center text-gray-500">▼</div>
              </div>

              <button type="submit" className="w-3/12 py-2 bg-white text-blue-700 font-semibold rounded-full hover:bg-gray-100 transition text-lg shadow-md active:scale-95">Register</button>
            </form>

            <p className="mt-6 text-sm">
              Sudah punya akun?{" "}
              <button onClick={onSwitchToLogin} className="bg-transparent p-0 text-white underline hover:text-blue-300 font-semibold">Login di sini</button>
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:hidden w-full items-center justify-center bg-blue-700 text-white px-6 py-10">
        <div className="max-w-md w-full">
          ...
          <form onSubmit={handleRegister} className="space-y-4 flex flex-col items-center w-full">
            ...
            {success && <p className="text-green-300 bg-green-800 bg-opacity-40 p-2 rounded text-center w-10/12">{success}</p>}

            <input type="text" placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} className="w-10/12 px-3 py-2 rounded-full border bg-white border-gray-300 text-black text-base" required />
            <input type="email" placeholder="E-Mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-10/12 px-3 py-2 rounded-full border bg-white border-gray-300 text-black text-base" required />
            ...
            <div className="relative w-10/12">
              <select value={role} onChange={(e) => setRole(e.target.value)} className="appearance-none w-full px-5 py-2 rounded-full border border-gray-300 bg-white text-black text-base">
                <option value="user">👤 User</option>
              </select>
              <div className="pointer-events-none absolute inset-y-0 right-6 flex items-center text-gray-500">▼</div>
            </div>

            <button type="submit" className="w-5/12 py-2 bg-white text-blue-700 font-semibold rounded-full hover:bg-gray-100 transition text-base shadow-md active:scale-95">Register</button>
          </form>
